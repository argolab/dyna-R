...
#from dyna.generic import Term, FVar, Rule, fresh
...
#        return Term('?', Term('&', x))   # suppress for now, but will be evaluated in the builin method
    ...
    def binop(self, x):
        ...
        return Term(renames.get(op,op), a,b)
...
#             return Term(str(op), a, b)
    ...
    def rules(self, rs):
        ...
        if self.HANDLE_DISJUNCTIONS:
            ...
            from dyna.normalizer import handle_disjunctions
    ...
    def string(self, x):
        [s] = x
        return Constant(s[1:-1].replace('\\"', '"'))  # remove outer quotes and escaped quotes

    def float(self, x):
        [a] = x
        return Constant(float(a))

    def int(self, x):
        [a] = x
        return Constant(int(a))
...
#    with None: #setunset(transformer, 'HANDLE_DISJUNCTIONS', handle_disjunctions):
    try:
