...
class DynaEngineWrapper:

    def has_agenda_work(self):
        return False
    ...
    def memoize_term(self, term, kind='none'):
        raise NotImplementedError()

    def get_defined_terms(self):
        return ()


class DynaRexpr1(DynaEngineWrapper):

    def __init__(self):
        from dyna import dyna_system, DynaSolverError
        from dyna.syntax.normalizer import add_rules, user_query
    ...
    def has_agenda_work(self):
        from dyna import dyna_system, DynaSolverError
        return bool(dyna_system.agenda)

    def run_agenda(self):
        from dyna import dyna_system, DynaSolverError
        dyna_system.run_agenda()

    def memoize_term(self, name, kind='none'):
        from dyna import dyna_system, DynaSolverError
        dyna_system.memoize_term(name, kind)

    def get_defined_terms(self):
        from dyna import dyna_system, DynaSolverError
        yield from dyna_system.terms_as_defined.keys()


class DynaTheoryMatchPaper(DynaEngineWrapper):

    def __init__(self):
        pass

    # there is currently no agenda in the paper version as everything is done via back chaining

    def add_rules(self, rule):
        from dyna_match_paper.syntax.normalizer import add_rules, user_query
        add_rules(rule)
...
class REPL:
    ...
    def __init__(self, interp, engine='default', args=None):
        self.interp = interp
        self.hist = hist
        self.hist_file = hist
        self.lineno = 0
        ...
        self.engine = engines[engine]  # load the engine that is selected
    ...
    def do_memoize_null(self, q):
        ...
        name, arity = q.split('/')
        arity = int(arity)
        self.engine.memoize_term((name, arity), kind='null')
    ...
    def do_memoize_unk(self, q):
        ...
        name, arity = q.split('/')
        arity = int(arity)
        self.engine.memoize_term((name, arity), kind='unk')
    ...
    def do_memoize_del(self, q):
        ...
        name, arity = q.split('/')
        arity = int(arity)
        self.engine.memoize_term((name, arity), kind='none')
    ...
    def cmdloop(self, _=None):
        ...
        class SlowCompleter(Completer):
            ...
            def get_completions(self, document, complete_event):
                ...
                words.update(name for (name, arity) in self.engine.get_defined_terms())
        ...
        while True:
            if self.engine.has_agenda_work() and not has_agenda_work and not self.suggested_prompt:
...
def main():
    ...
    argp = ArgumentParser()
    argp.add_argument('--engine', default='default', choices=list(engines.keys()),
                      help='Select which Backend is used for evaluating expressions')

    argp = argp.parse_args()

    repl = REPL(dyna_system, engine=argp.engine)
