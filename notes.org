...
* distribution
+ looks like the backend / most of the implementation is going to end up in
  java/clojure.  It would be nice if there was some way to integrate this into
  the python eco system for machine learning applications.  As noted above, the
  ~distutils.setup_keywords~ method and ~pkg_resources.iter_entry_points~ would
  let the system hook into python's packaging system.  It would be nice of there
  was some way in which we overloaded python's import and then allowed for .dyna
  files to be directly imported.  The question would then be how would those
  operations map into the procedural operations which are performed by python.
  If it had all of the methods exposed to python, then that could allow that to
  be easily used.  This would be like a totally different backend which is
  transporting python values to the JVM and back.  If the system was efficient
  enough then

* AST definition
** nodes -- which can come out of the parser directly
+ for representing expressions which should get passed into the compiler via special operations
  - ~:- dispose foo(quote1)~ => ~$compiler_expression(dispose(foo(quote1)))~
+ $compiler_expression(Term)
+ $define_term(Head_of_term, Dynabase_object_or_null, Aggregator_as_string, Body_of_term)
  + The head of the term will be the term structure with the names of variables.  In the case of
+ $with_key
+ $constant
+ $variable
+ $dynabase_create
+ $dynabase_call
  - if there is a call which is to a dynabase with some qualified variable being used.
  - implicit dynabase calls are problematic as everything is represented as a call, and because of the dynamic nature of a dynabase, we can
+ $quote1
+ $anon_variable
** nodes -- which include intermediate nodes
+ $define_term_normalized(Name_of_term_string, Arity_of_term_int, File_name_of_term, aggregator Body_of_term).
+ $aggregator(resulting-variable, incoming-variable, aggregator-method, projected-out-variables,


** meta programming
+ there needs to be some way of having an escaped variable.  The variable will
  then want to match whatever is the surrounding context.  That escaped variable
  would ideally have whatever the expression would represent.
+ the escaped variable could go into any place where a variable exists, but in
  some places ,there are terms which are being created.  How would those terms
  work in the representation
+ If there is something like <!{ }> then that would allow for it to make the
  different expression.

+ would then like to have that there is some expression which corresponds with
  what operations are going to be represented.
+ how would a quoted variable get used in the case of a string.  There

* web demo?
https://leaningtech.com/cheerpj/ -- could compile it into something that can run in the browser?
