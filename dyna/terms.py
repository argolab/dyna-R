...
def simplify_buildStructure(self, frame):
    if self.result.isBound(frame):
        ...
        if not isinstance(res, Term) or res.name != self.name or res.arity != len(self.arguments):
            return Terminal(0)  # then this has failed
        for i, var in enumerate(self.arguments):
            var.setValue(frame, res.get_argument(i))
...
def simplify_reflectStructure(self, frame):
    if self.result.isBound(frame):
        ...
        self.args_list.setValue(frame, res.make_list())
        self.num_args.setValue(frame, res.arity)
        return Terminal(1)
...
def simplify_evaluate(self, frame):
    if self.term_var.isBound(frame):
        ...
        if not isinstance(t, Term):
            return Terminal(0)
        r = self.dyna_system.call_term(t.name, t.arity+len(self.extra_args))(*(constant(a) for a in t.make_pylist()), *self.extra_args, ret=self.ret)
        return simplify(r, frame)
