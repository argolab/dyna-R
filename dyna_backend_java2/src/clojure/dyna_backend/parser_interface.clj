...
(defn -make_rexpr [^dyna_backend.DynaParserInterfaceImpl this ^String name args]
  (apply construct-rexpr name args))

(defn -make_variable [^dyna_backend.DynaParserInterfaceImpl this ^String name]
  (make-variable name))

(defn -make_unnamed_variable [^dyna_backend.DynaParserInterfaceImpl this]
...
(defn -make_constant [^dyna_backend.DynaParserInterfaceImpl this value]
  (make-constant value))
...
(defn- get-with-default-var [^dyna_backend.DynaParserInterfaceImpl this path]
...
(defn- named-varibles [^dyna_backend.DynaParserInterfaceImpl this]
...
(defn -make_call [^dyna_backend.DynaParserInterfaceImpl this ^String name args]
...
(defn -make_structure [^dyna_backend.DynaParserInterfaceImpl this ^String name args]
  (let [ret-var (make-variable (gensym))
        rexpr (make-unify ret-var (make-structured-value name (vec args)))]
    (add-rexpr this rexpr)
    ret-var))

(defn -make_rexpr_add [^dyna_backend.DynaParserInterfaceImpl this ^String name args]
  (let [rexpr (apply construct-rexpr name args)]
    (add-rexpr this rexpr)
    rexpr))

(defn -unify_with_true [^dyna_backend.DynaParserInterfaceImpl this value]
  (add-rexpr this (make-unify value (make-constant true))))

;; (defn -enter_dynabase_context [^dyna_backend.DynaParserInterfaceImpl this]
;;   (dyna_backend.DynaParserInterfaceImpl. (assoc (deref (.state this))
;;                                :current-dynbase (gensym)  ;; this will want to have
;;                                )))

(defn -copy_interface [^dyna_backend.DynaParserInterfaceImpl this]
  (dyna_backend.DynaParserInterfaceImpl. (assoc (deref (.state this))
                                                :parent-parser this
                                                )))

;; (defn -set_atom [^dyna_backend.DynaParserInterfaceImpl this ^String name arguments ^String aggregator]
;;   (update-interface this )

;;   (swap! (.state this) assoc :current-atom {:name name :arguments (vec arguments) :aggregator aggregator}))

;; (defn -construct_atom [^dyna_backend.DynaParserInterfaceImpl this return-value]
;;   (???))
...
(defn -set_atom_args [^dyna_backend.DynaParserInterfaceImpl this args]
...
(defn -set_atom_aggregator [^dyna_backend.DynaParserInterfaceImpl this ^String name]
  (assert (string? name))
  (swap! (.state this) assoc-in [:current-atom :aggregator] name))

(defn -set_atom_result_variable [^dyna_backend.DynaParserInterfaceImpl this value]
...
(defn -set_dynabase_inherits_variable [^dyna_backend.DynaParserInterfaceImpl this var]
  (swap! (.state this) assoc-in [:current-dynbase :inherits-from-variable] var))

(defn -get_dynabase_construct_variable [^dyna_backend.DynaParserInterfaceImpl this]
...
(defn -finish_dynabase [^dyna_backend.DynaParserInterfaceImpl this]
  (let [state @(.state this)
        process (fn [state]
...
(defn -run_query [^dyna_backend.DynaParserInterfaceImpl this rexpr]
